...
double StudentWorld::ed(double x1, double y1, double x2, double y2){
...
void StudentWorld::collElement(Actor* cab) {
    
    list<Actor*>::iterator it = result.begin();
    while(it != result.end()) {
        if((*it)->isDamageable()){
            if(ed(cab->getX(),cab->getY(),(*it)->getX(),(*it)->getY())
...
void StudentWorld::collTorpedo(Actor* c) {
    ...
    while(it != result.end()) {
        if((*it)->isDamageable()){
            if(ed(c->getX(),c->getY(),(*it)->getX(),(*it)->getY())
...
int StudentWorld::move()
{
    ...
    res << " Score: " << getScore();
    res << "  Level: " << getLevel();
    res << "  Cabbages: " <<(int)((player->getEpoints() / 30.0) * 100)<< "%";
