...
class DamObj: public Actor
{
public:
    DamObj(int ID, double x, double y, StudentWorld* s, int dir, double size, int depth,
    ...
    virtual int getCPoints() { return cEnergyPoints;}
...
class NachenBlaster: public DamObj
{
public:
    NachenBlaster(StudentWorld*s, double x = 0, double y = 128, double size = 1, int dir = 0, int depth = 0)
    :DamObj(IID_NACHENBLASTER, x, y, s, dir, size, depth,50,30), torpedo_count(0){
        // setEpoints(30);
        //setHpoints(50);
    };
    virtual void doSomething();
